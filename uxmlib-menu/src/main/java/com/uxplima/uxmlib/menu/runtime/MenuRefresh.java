...
/**
 * Starts the single repeating task that re-renders an open menu. A menu whose spec disables refresh starts no task at
 * all, so a static menu costs nothing beyond the open. When refresh is enabled the task runs on the global region
 * thread at the spec's cadence, and its cancel handle is parked on the holder: the holder owns teardown, so closing or
 * quitting the menu cancels exactly this task and never leaks a timer.
 */
