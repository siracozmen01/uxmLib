 ...
 * component, enchantment, name, and lore the operator captured. It reads the token through {@link SerializedItems},
 * the one place that knows what a written-down item looks like, so this provider and whatever wrote the token
 * cannot drift apart.
 *
 ...
 * spec lore is added beneath it.
 *
 * <p>A token that will not read is caught here rather than left to travel. {@link SerializedItems#decode} raises
 * it, because from its position damage and a plain material name must not look alike; from this position they may,
 * because a menu being drawn has to finish being drawn. So the icon falls through to the renderer's material
 * fallback, and the window opens missing one tile instead of not opening at all.
 *
