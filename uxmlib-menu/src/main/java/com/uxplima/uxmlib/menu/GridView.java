...
/**
 * The handle the engine hands a grid's click handler so it can repaint the open canvas after mutating whatever model it
 * draws from. A grid stays open across clicks (unlike a confirm or selector, which close on the first choice), so a
 * place / move / clear that changes the backing edit session needs the window redrawn in place rather than reopened.
