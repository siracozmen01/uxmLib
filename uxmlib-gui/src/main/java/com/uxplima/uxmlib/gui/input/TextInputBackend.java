 ...
 * resolved prompt {@link Component} (so neither touches the catalog) and report a raw {@link InputResult} exactly once;
 * {@link TextInput} owns backend selection and the cancel-keyword policy, so a backend only has to capture text and
 * hand it back.
 */
@NullMarked
